...
import { PatientBanner, FlowBar, RefNums, Card, Sec, FL, ErrBox, EmptyState, BtnGhost, BtnGreen, BtnPrimary, IS, SS, TA, Btn } from "../../components/common/HMSComponents";
...
export default function DoctorClerking() {
  ...
  const handleSave = () => {
    ...
    saveClerking(selQNo, clk, labSel, rxList, labUrg, labNote,
      () => { setSelQNo(null); navigate("/hms/queue"); });
  };
  ...
  return (
    <HMSLayout>
      <HMSTopBar
        title="Doctor Clerking"
        subtitle={active ? active.queueNo + " · " + (active.firstName||active.name) + " " + (active.lastName||"") : waiting.length + " patient(s) waiting"}
        action={
      ...
      <div style={{padding:"20px 24px"}}>
        {!active ? (
          waiting.length===0 ? <EmptyState icon="🩻" msg="No patients waiting for doctor." /> :
          waiting.map(p => {
            const isReturn = p.status==="With Doctor (Post-Lab)";
            let crit=0,abnorm=0;
            if (p.clerking?.labResults) Object.values(p.clerking.labResults).forEach(r=>{ if(r.flag==="critical") crit++; else if(r.flag==="low"||r.flag==="high") abnorm++; });
            return (
              <div key={p.queueNo} onClick={()=>pickPatient(p)} style={{
                background:T.card,borderRadius:11,padding:"14px 18px",marginBottom:10,
                boxShadow:"0 1px 6px rgba(0,0,0,.06)",border:"1.5px solid "+(isReturn?"#99f6e4":T.border),
                cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"
              }}>
                <div>
                  <div style={{fontWeight:700,fontSize:14}}>{p.firstName||p.name} {p.lastName||""}</div>
                  <div style={{fontSize:11,color:T.slateL,fontFamily:"'DM Mono',monospace"}}>{p.queueNo} · {p.triage?.complaint||""}</div>
                  {isReturn && (
                    <div style={{display:"flex",gap:6,marginTop:6}}>
                      <span style={{background:"#cffafe",color:"#0e7490",borderRadius:5,padding:"1px 8px",fontSize:10,fontWeight:700}}>🧪 Lab results available</span>
                      {crit>0 && <span style={{background:"#fee2e2",color:T.red,borderRadius:5,padding:"1px 8px",fontSize:10,fontWeight:700}}>🔴 {crit} critical</span>}
                      {abnorm>0 && <span style={{background:"#fef3c7",color:"#b45309",borderRadius:5,padding:"1px 8px",fontSize:10,fontWeight:700}}>⚠️ {abnorm} abnormal</span>}
                    </div>
                  )}
                </div>
                <button style={{...Btn,padding:"8px 16px",fontSize:12,background:isReturn?T.teal:T.purple,color:"#fff"}}>
                  {isReturn?"🧪 Review Results":"🩻 See Patient"}
                </button>
              </div>
            );
          })
                ...
                <div>
                  ...
                  {labSel.length>0 ? (
                    <div style={{border:"1.5px solid "+(resulted?"#99f6e4":"#93c5fd"),borderRadius:10,overflow:"hidden"}}>
                      ...
                      {labSel.map((id,i)=>{const t=LAB_TESTS.find(x=>x.id===id);if(!t)return null;return(
