         ...
         return (
            <React.Fragment key={label}>
               <div className="flex flex-col items-center gap-1.5">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm transition-all border-2 ${isDone ? 'bg-primary border-primary text-white' :
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {PATIENT_TYPES.map(pt => (
                           <button key={pt.id} type="button" onClick={() => { setPatientType(pt.id); setPatient(null); }}
                              className={`flex flex-col items-center justify-center gap-6 p-10 min-h-[240px] rounded-3xl border-2 transition-all text-center ${patientType === pt.id
                              ...
                              <div className={`p-5 rounded-2xl ${patientType === pt.id
                        ...
                        {(() => {
                           ...
                           return visible.map(svc => (
                              <button type="button" key={svc.id}
                                 ...
                                 className={`flex flex-col items-center gap-3 p-6 rounded-2xl border-2 transition-all text-center group ${selectedService === svc.id
                           ...
                           <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                              {SPECIALTIES.map(sp => (
                                 <button type="button" key={sp.id} onClick={() => setSelectedSpecialty(sp.id)}
                                    className={`flex items-center gap-3 px-3 py-3 rounded-xl border-2 text-sm font-bold transition-all ${selectedSpecialty === sp.id
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 content-start">
                        {PAYMENT_METHODS.map(pm => (
                           <button type="button" key={pm.id} onClick={() => setPaymentMethod(pm.id)}
                              className={`flex items-center gap-4 p-5 rounded-xl border-2 transition-all text-left ${paymentMethod === pm.id
                                    ? pm.color === 'green' ? 'border-green-500 bg-green-50' : 'border-blue-500 bg-blue-50'
                                    : 'border-silver-200 hover:border-silver-400 bg-white'
                                 }`}>
                              <div className={`p-3 rounded-xl ${paymentMethod === pm.id
