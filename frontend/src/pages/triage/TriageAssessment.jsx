...
export default function TriageAssessment() {
  ...
  const pickPatient = (p) => {
    ...
    setTrForm(p.triage ? { ...p.triage } : { ...INIT_FORM, time: timeNow() });
    setTrErr("");
  };
  ...
  const handleSave = () => {
    ...
    saveTriage(selectedQueueNo, trForm, () => { setSelectedQueueNo(null); navigate("/hms/queue"); });
  };
  ...
  return (
    <HMSLayout>
      ...
      <div style={{ padding: "20px 24px" }}>
        {!active ? (
          waiting.length === 0 ? <EmptyState icon="🩺" msg="No patients awaiting triage." /> :
          waiting.map(p => (
            <div key={p.queueNo} onClick={() => pickPatient(p)} style={{
              ...
              <span style={{ ...BtnGhost, padding: "7px 16px", fontSize: 12, color: "#ea580c", borderColor: "#ea580c" }}>🩺 Triage</span>
            </div>
          ))
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 16, alignItems: "start" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <button onClick={() => setSelectedQueueNo(null)} style={BtnGhost}>← Back</button>
